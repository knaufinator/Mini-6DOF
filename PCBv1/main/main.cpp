...
// ── Input Scaling ────────────────────────────────────────────────────
// 14-bit default gives 16384 discrete steps — 4× finer than 12-bit
// for smoother motion on the small-geometry mini platform.
static uint8_t inputBitRange = 14;
static float maxRawInput = 16383.0f;
...
// Units: physical (mm or rad) per call. At 200Hz loop rate this is per 5ms.
// Tuned for mini platform: 2mm/5ms = 400mm/s — fast enough for BLE accel,
// smooth enough to avoid servo jitter on the small geometry.
#define SLEW_RATE_MAX   2.0f                             // mm (or rad) per 5ms cycle
...
// Convert microseconds to LEDC duty value (integer math, no float division)
// At 50Hz: period = 20000µs, max_duty = 65535
// duty = us * 65535 / 20000 ≈ us * 3277 / 1000 (exact enough, <0.05% error)
static inline uint32_t usToDuty(int us) {
    return (uint32_t)((uint64_t)us * LEDC_TIMER_MAX / 20000);
}
...
extern "C" void app_main(void) {
    ...
    // ── Main Loop ──────────────────────────────────────────────────────
    // Motion processing runs at 200Hz (5ms) for responsive servo control.
    // Telemetry runs at its own configurable rate (default 50Hz).
    // This decoupling ensures BLE accel data is processed promptly even
    // when telemetry is throttled for bandwidth.
    #define MOTION_LOOP_MS  5   // 200Hz servo update rate
    int64_t lastTelemetryUs = 0;
    int64_t telemetryIntervalUs = (int64_t)telemetryDelayMs * 1000;
    ...
    for (;;) {
        ...
        // ── Telemetry at independent rate ──
        // Stays silent until app sends TELRATE:N after handshake.
        telemetryIntervalUs = (int64_t)telemetryDelayMs * 1000;
        if (telemetryEnabled && (now - lastTelemetryUs) >= telemetryIntervalUs) {
            lastTelemetryUs = now;
            if (xPrintMutex && xSemaphoreTake(xPrintMutex, 0) == pdTRUE) {
